...
USER_AGENT = os.environ.get(
    ...
    "LiuHaiyangJournalMonitor/1.0 (mailto:haiyang.liu.q6@elms.hokudai.ac.jp)",
)
MAX_LANDING_FETCHES = int(os.environ.get("JOURNAL_UPDATES_MAX_ENRICH", "20"))
OPENALEX_BATCH_SIZE = int(os.environ.get("JOURNAL_UPDATES_OPENALEX_BATCH", "40"))
...
def main() -> int:
    ...
    try:
        ...
        print(f"Updated journal feed with {len(combined)} articles.")
        return 0
    except (HTTPError, URLError, TimeoutError, ValueError) as error:
        if DATA_PATH.exists():
            print(f"Keeping existing journal data due to fetch error: {error}")
            return 0
        print(f"Failed to update journal feed: {error}", file=sys.stderr)
        return 1
