...
DATA_PATH = REPO_ROOT / "data" / "cityscope_updates.json"
RANGE_START = "2026-01-01"
CROSSREF_API = "https://api.crossref.org/journals/{issn}/works"
USER_AGENT = os.environ.get(
    "CITYSCOPE_USER_AGENT",
    "LiuHaiyangCityScopeMonitor/1.0 (mailto:haiyang.liu.q6@elms.hokudai.ac.jp)",
)
MAX_LANDING_FETCHES = int(os.environ.get("CITYSCOPE_MAX_ENRICH", "20"))
OPENALEX_BATCH_SIZE = int(os.environ.get("CITYSCOPE_OPENALEX_BATCH", "40"))

JOURNALS = [
    {
        "id": "nature-cities",
        "name": "Nature Cities",
        "short_name": "Nature Cities",
        "issns": ["2731-9997"],
    },
    {
        "id": "computers-environment-urban-systems",
        "name": "Computers, Environment and Urban Systems",
        "short_name": "Computers, Environment and Urban Systems",
        "issns": ["0198-9715"],
    },
    {
        "id": "landscape-urban-planning",
        "name": "Landscape and Urban Planning",
        "short_name": "Landscape and Urban Planning",
        "issns": ["0169-2046"],
    },
    {
        "id": "cities",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Updated CityScope feed with {len(combined)} articles.")
        return 0
    except (HTTPError, URLError, TimeoutError, ValueError) as error:
        if DATA_PATH.exists():
            print(f"Keeping existing CityScope data due to fetch error: {error}")
            return 0
        print(f"Failed to update CityScope feed: {error}", file=sys.stderr)
        return 1
